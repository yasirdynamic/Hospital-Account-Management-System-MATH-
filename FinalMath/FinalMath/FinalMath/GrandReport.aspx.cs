...
namespace FinalMath
{
    public partial class GrandReport : System.Web.UI.Page
    {
        ...
        protected void btnshowreport_Click(object sender, EventArgs e)
        {
            ...
            if (ddlincomeandexpense.SelectedItem.Text != "All")
            {
            ...
            cmd = new SqlCommand("select duestype,exp_type,i.DATE as 'Date',i.VOUCHER_NO as 'Voucher',i.DESCRIPTION as 'Desc',INCOME_AMOUNT as 'CrRs',EXPENSE_AMOUNT as 'DrRs',i.PIC as 'Doc',v.VENDOR_NAME as 'Vendor',h.HEAD_NAME as 'Head',m.MODES_OF_PAYMENT_NAME as 'Mode',u.USER_NAME as 'username' from DUES i inner join VENDORS v on i.VENDOR_FID=v.VENDOR_ID inner join USERS u on i.USER_FID=u.USER_ID inner join HEADS h on h.HEAD_ID=i.HEAD_FID inner join MODES_OF_PAYMENTS m on m.MODES_OF_PAYMENTS_ID=i.MODES_OF_PAYMENTS_FID  WHERE 1=1 " + filter + " ORDER BY i.DATE ASC", con);
            da.SelectCommand = cmd;
            da.Fill(dt);
        ...
        {
            string message = "Back is created successfully!!!";
            try
            {
                string dbNAme = "DBMA";
                string backupDestination = Server.MapPath("~/DbBackup");
                if (!Directory.Exists(backupDestination))
                {
                    Directory.CreateDirectory(backupDestination);
                }
                string fileName = dbNAme + " of " + DateTime.Now.ToString("yyyy-MM-dd") + ".bak";
                string conString = "data source= 95.217.141.238;database=DBMA;user id=dbmath;password=xpIm43xpIm43xpIm43";
